...
class ChatServer(object):
    def __init__(self, host, port):
        ...
        try:
            self.db = psycopg2.connect(
                "dbname='chat_db' user='postgres' host='localhost' password='***'")
    ...
    def _input_for_logging(self, client, request):
        if request.command_type == ChatRequest.SIGN_UP:
            response = ChatResponse()
            ...
            if self._is_user_in_db(request.sign.login):
                ...
                if request.sign.passwd and request.sign.login:
                    self._add_user_to_db(
                        request.sign.login, request.sign.passwd)
                    response.successful = True
            ...
            if self._is_user_in_db(
                    request.sign.login) and self._login_user(
                    client,
                    request.sign.login,
                    request.sign.passwd,
                    request.sign.hidden):
    ...
    def _input_with_logged_user(self, client, request):
        if request.command_type == ChatRequest.ADD_CHAT:
            ...
            if request.info_text != '' and request.info_text.isdigit():
                ...
                limit = 10
            self._get_messages_to_response(
                response.messages, limit, client_id=client.id)
            response.successful = True
            self._send_message_to_client(client, response.SerializeToString())
    ...
    def _login_user(self, client, login, passwd, hidden):
        ...
        cursor = self.db.cursor()
        cursor.execute(
            "SELECT * FROM users WHERE name = %s and password = %s;", (login, passwd,))
        rows = cursor.fetchall()
    ...
    def _add_users_to_chat(self, client, args):
        ...
        if len(rows):
            ...
            for user in names:
                ...
                user_id = rows[0][0]
                cursor.execute(
                    "INSERT INTO chat_users (user_id, chat_id, hidden) "
                    "VALUES (%s, %s, %s);", (user_id, chat_id, False))
    ...
    def _get_messages_by_chat(self, chat_id, limit):
        cursor = self.db.cursor()
        cursor.execute(
            "SELECT 	messages.message_id, chats.chat_id AS chat, users.name AS author, "
    ...
    def _preparing_message_for_db(self, client, request):
        ...
        mes.time = datetime.datetime.fromtimestamp(
    ...
    def _send_chat(self, user_id, chat_id):
        ...
        self._get_messages_to_response(
            response.messages, 10, chats=[[chat_id]])
        response = response.SerializeToString()
        for client in self.connected_clients:
